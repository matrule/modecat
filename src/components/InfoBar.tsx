...
export function InfoBar() {
  ...
  return (
    <div className="info-bar">
      {/* Row 1 — transport counters (read/write) */}
      <div className="info-bar__row">
        <span className="info-bar__status">
          {transport.playing ? (
            <span className="info-bar__play">▶ PLAY</span>
          ) : (
            <span className="info-bar__stop">■ STOP</span>
          )}
        </span>
      ...
      {/* Row 2 — position counters (read-only) */}
      <div className="info-bar__row info-bar__row--secondary">
        <span className="label">Sg</span>
        <span className="pill">{transport.songPos}</span>

        <span className="label">Sc</span>
        <span className="pill">{song.positions.length}</span>

        <span className="label">B</span>
        <span className="pill">{activePid ?? '--'}</span>

        <span className="label">Trks</span>
